	...
	*/	
	...
	function gdlr_soccer_get_match_info($match_options = array(), $additional = ''){ 
		...
		if( !empty($match_options['date-of-match']) || !empty($match_options['location']) ){
			...
			if( !empty($match_options['date-of-match']) ){
				...
				$match_time = empty($match_options['match-time'])? '00:00': substr($match_options['match-time'], 0, 5);
				$match_date = strtotime($match_options['date-of-match'] . 'T' . $match_time . ':00');		
	...
	function gdlr_print_result_by_month($query){
		...
		while( $query->have_posts() ){ $query->the_post();
			...
			echo '<span class="gdlr-result-match-team gdlr-left">' . $match_options['home'] . '</span>';
			...
			echo '<span class="gdlr-result-match-team gdlr-right">' . $match_options['away'] . '</span>';
	...
	function gdlr_print_upcoming_match_item( $settings ){
		...
		while( $query->have_posts() ){ $query->the_post();
			...
			echo '<span class="gdlr-upcoming-match-team gdlr-left">' . $match_options['home'] . '</span>';
			echo '<span class="gdlr-upcoming-match-versus">' . __('VS', 'gdlr-soccer') . '</span>';
			echo '<span class="gdlr-upcoming-match-team gdlr-right">' . $match_options['away'] . '</span>';
